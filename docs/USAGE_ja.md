...
最初に`gmaginai-l`に異世界の創造者のインストールを登録するためにプロファイルを作成する必要があります。  
...
「ヒント:…」メッセージは、配布zipファイルそれ自体はModではないため、注意として表示されるメッセージです。  
